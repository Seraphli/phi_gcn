...
class RolloutStorage(object):
    def __init__(
        ...
        self.step = 0
        self.minari_loader = minari_loader
    ...
    def compute_returns(self, next_value, use_gae, gamma, tau, gcn, alpha):
        ...
        gcn_phi = torch.exp(
            gcn(self.hidden_states.reshape(total_len, self.hidden_states.size(2)), adj)
        )
        gcn_phi = (
            gcn_phi[:, 1]
            .reshape(self.hidden_states.size(0), self.hidden_states.size(1), 1)
            .detach()
        )
        
        # Compute offline phi using minari loader
        offline_phi = torch.zeros_like(gcn_phi)
        if self.minari_loader is not None:
            for t in range(self.obs.size(0) - 1):  # -1 because obs has num_steps + 1
                for n in range(self.obs.size(1)):
                    obs = self.obs[t, n].cpu().numpy()
                    offline_value = self.minari_loader.get_state_value(obs)
                    offline_phi[t, n, 0] = float(offline_value)
        if use_gae:
            ...
            for step in reversed(range(self.rewards.size(0))):
                ...
                if step == (len(self.rewards) - 1):
                    ...
                    delta = (
                        self.rewards[step]
                        + gamma * gcn_phi[step + 1] * self.masks[step + 1]
                        - gcn_phi[step]
                    )
            ...
            for step in reversed(range(self.rewards.size(0))):
                self.returns[step] = (
                    ...
                    + self.rewards[step]
                    + (1 - alpha)
                    * (gamma * self.masks[step + 1] * gcn_phi[step + 1] - gcn_phi[step])
                )

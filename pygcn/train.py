...
def update_graph(model, optimizer, features, adj, rew_states, args, envs):
    if adj.shape[0] > 1:
        ...
        for r_s in rew_states:
            if len(envs.observation_space.shape) == 1:  # MuJoCo experiments
                ...
                labels[r_s[0]] = (
                    torch.tensor([1.0]) if r_s[1] > 0.0 else torch.tensor([0.0])
                )
    ...
    return total_loss / args.gcn_epochs
